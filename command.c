...
T_Command get_user_input(char* param, char* input)
{
  ...
  *param = '\0';
  ...
  while(cmd == UNKNOWN && i < N_CMD)
  {
    ...
    if (!strcasecmp(toks, short_cmd_to_str[i]) || !strcasecmp(toks, cmd_to_str[i]))
    {
      ...
      else
      {
        if (cmd == GET || cmd == DROP)
        {
          /* Compara con los objetos, sin tener en cuenta mayúsculas/minúsculas */
          if (strcasecmp(toks,"O1")!=0 && strcasecmp(toks,"O2")!=0 &&
              strcasecmp(toks,"O3")!=0 && strcasecmp(toks,"O4")!=0)
          {
            return UNKNOWN;
          }
          strcpy(param, toks);
  ...
  /* Devuelve el comando introducido */
  return cmd;
}
